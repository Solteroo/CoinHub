...
const ACCENT = "#f43f5e";
function getColor(n: number) { return n === 0 ? "green" : RED.has(n) ? "red" : "black"; }
      ...
      >
            ...
            <div className="absolute inset-2 rounded-full overflow-hidden">
              <svg viewBox="0 0 100 100" className="w-full h-full">
                {Array.from({ length: 37 }, (_, i) => {
                  ...
                  const x1 = 50 + 48 * Math.cos(rad - Math.PI/2); const y1 = 50 + 48 * Math.sin(rad - Math.PI/2);
                  const x2 = 50 + 48 * Math.cos(rad2 - Math.PI/2); const y2 = 50 + 48 * Math.sin(rad2 - Math.PI/2);
                  const c = getColor(i);
                  return <path key={i} d={`M 50 50 L ${x1} ${y1} A 48 48 0 0 1 ${x2} ${y2} Z`} fill={c === "red" ? "#991b1b" : c === "green" ? "#166534" : "#1f2937"} stroke="rgba(0,0,0,0.5)" strokeWidth="0.5" />;
                })}
        ...
        <div className="shrink-0 overflow-hidden rounded-xl border border-white/5" style={{ background: "rgba(255,255,255,0.03)" }}>
          <div className="grid" style={{ gridTemplateColumns: "repeat(13, 1fr)", gap: "1px" }}>
            ...
            <div className={cn("aspect-square flex items-center justify-center text-[7px] font-bold col-span-1", result?.number === 0 && "ring-1 ring-white")}
