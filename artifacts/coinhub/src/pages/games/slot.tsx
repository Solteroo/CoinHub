  ...
  const handleSpin = async () => {
    ...
    playSlot.mutate({ data: { bet } }, {
      onSuccess: async (result) => {
        // Wait at least some time for dramatic effect
        ...
        if (result.won > 0) {
          ...
          toast({
            title: result.label,
            description: `${result.won} teňňe gazandyňyz! (${result.multiplier}x)`,
          });
        }
    ...
    <Layout hideNav>
      <div className="p-4 flex flex-col items-center min-h-[calc(100vh-80px)] pb-24">
        ...
        <div className="mb-8 text-center">
          <h1 className="text-4xl font-black italic gold-text-gradient uppercase tracking-tighter drop-shadow-sm">SLOT MAŞYN</h1>
               ...
               <div key={i} className="bg-background/80 rounded-xl border-2 border-primary/10 overflow-hidden relative">
                 ...
                 >
                   <span className={cn(
                     ...
                     reels[i] === "7" ? "text-red-500" : "gold-text-gradient"
                   )}>
                     {reels[i]}
                   </span>
                 </motion.div>
        ...
        <div className="w-full max-w-sm space-y-6">
          ...
          {history.length > 0 && (
            <div className="space-y-2">
               <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-[0.2em] px-1">Soňky oýunlar</p>
          ...
          <Accordion type="single" collapsible className="w-full">
            <AccordionItem value="paytable" className="border-primary/10">
              <AccordionTrigger className="text-xs font-bold uppercase tracking-widest text-primary/70 hover:text-primary transition-colors">Utuş tablisasy</AccordionTrigger>
              <AccordionContent className="space-y-2">
                ...
                <PayRow label="Islendik 2 sany" value="1.3x" />
              </AccordionContent>
            </AccordionItem>
          </Accordion>
        </div>
