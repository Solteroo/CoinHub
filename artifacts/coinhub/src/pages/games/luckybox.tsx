...
export default function LuckyBoxGame() {
  ...
  const handleOpen = (index: number) => {
    ...
    setPlaying(true); setPickedIndex(index); playClick();
    playBox.mutate({ data: { bet, pickIndex: index } }, {
      onSuccess: (res) => {
        setResult(res);
        if (res.won > 0) {
          playWin();
          if (res.multiplier >= 10) confetti({ particleCount: 150, spread: 70, origin: { y: 0.5 }, colors: [ACCENT, "#fde68a"] });
      ...
      onError: (err: any) => { setPlaying(false); setPickedIndex(null); toast({ title: t("error"), description: err.message, variant: "destructive" }); },
    });
  };

  const reset = () => { if (playing) return; setPickedIndex(null); setResult(null); };
      ...
      >
        ...
        {pickedIndex === null && (
          <motion.p animate={{ opacity: [0.5, 1, 0.5] }} transition={{ duration: 2, repeat: Infinity }}
            className="shrink-0 text-center text-xs font-black uppercase tracking-[0.2em]" style={{ color: ACCENT }}>
            📦 {t("choose_box")}
          </motion.p>
        )}

        {/* Box grid — fills remaining space */}
                  ...
                  {!isRevealed ? (
                    <motion.button
                      ...
                      style={{
                        ...
                        boxShadow: isPicked ? `0 0 28px ${ACCENT}45` : undefined,
                      }}
                    >
                    ...
                    <motion.div
                      ...
                      style={{
                        background: isPicked && hasWin ? "rgba(52,211,153,0.15)" : isPicked && !hasWin ? "rgba(239,68,68,0.15)" : !isPicked && hasWin ? `${ACCENT}10` : "rgba(255,255,255,0.03)",
                        borderColor: isPicked && hasWin ? "rgba(52,211,153,0.5)" : isPicked && !hasWin ? "rgba(239,68,68,0.4)" : !isPicked && hasWin ? `${ACCENT}40` : "rgba(255,255,255,0.06)",
                        boxShadow: isPicked && hasWin ? "0 0 25px rgba(52,211,153,0.3)" : isPicked && !hasWin ? "0 0 20px rgba(239,68,68,0.2)" : undefined,
                      }}
                    >
                      {reveal && (
                        <>
