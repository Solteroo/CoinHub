...
export default function CrashGame() {
  ...
  const finalizeGame = (res: any) => {
    ...
    if (res.cashedOut) {
      ...
      toast({
        title: "Gyzgyn utuş!",
        description: `+${res.netChange} teňňe gazandyňyz!`,
      });
    } else {
      setStatus("crashed");
      ...
      toast({
        title: "Partlady",
        description: `${res.bet} teňňe ýitirildi.`,
        variant: "destructive"
      });
    }
      ...
      <div className="p-4 flex flex-col items-center min-h-[calc(100vh-80px)] pb-24">
        ...
        <div className="w-full max-w-sm space-y-6">
          <div className="space-y-2">
            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest px-1">Goýum mukdary</p>
            <BetSelector value={bet} onChange={setBet} min={10} max={user?.coins ?? 0} disabled={playing} />
          </div>

          <div className="space-y-3">
            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest px-1">Awtomatiki nagtlaşdyrmak</p>
          ...
          >
            {playing ? "UÇUŞDA..." : "BAŞLA"}
          </Button>

          {/* History */}
          {history.length > 0 && (
            <div className="space-y-2">
              <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest px-1">Soňky raundlar</p>
