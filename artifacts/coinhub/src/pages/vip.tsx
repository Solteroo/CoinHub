      ...
      <div className="p-4 space-y-6 pb-24">
        <div className="relative overflow-hidden bg-gradient-to-br from-primary/20 via-card to-card border border-primary/40 rounded-3xl p-8 text-center gold-glow">
          ...
          <div className="relative">
            ...
            <h1 className="text-3xl font-black italic gold-text-gradient uppercase tracking-tighter">{t("vip_benefits")}</h1>
            <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest mt-2">{t("vip_subtitle")}</p>
          </div>
        </div>

        <div className="bg-card border border-primary/15 rounded-2xl p-6 space-y-3">
          <h2 className="text-sm font-bold uppercase tracking-widest text-white mb-3">{t("vip_whats_in")}</h2>
          {BENEFITS.map((b, i) => (
        ...
        <div className="bg-card border border-primary/30 rounded-2xl p-5 space-y-3">
          <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground">{t("vip_to_order")}</p>
          {owner ? (
            <Link href={`/dm/${owner.id}`}>
              <button className="w-full h-14 rounded-xl gold-gradient text-black font-black uppercase tracking-widest text-sm flex items-center justify-center gap-2 active:scale-[0.98]">
                <MessageCircle className="w-5 h-5" /> {t("write_to_owner")}
              </button>
            </Link>
          ) : (
            <p className="text-xs text-muted-foreground text-center py-2">{t("owner_not_set")}</p>
          )}
          <p className="text-[10px] text-muted-foreground text-center">{t("vip_contact_desc")}</p>
        </div>
      </div>
    </Layout>
  );
